...
void OnTick()
  {
   ...
   string current_pair = "";
   string names = "";
   // Get the systems that are supposed to be included for this currency pair
   int systems = FileOpen(current_pair, FILE_READ|FILE_WRITE|FILE_ANSI, '\t', CP_ACP);
   if(systems != INVALID_HANDLE)
   {
      while(!FileIsEnding(systems)) 
      {
         names = names+"|&&|"+systems; 
      }
   }
   ...
   int k=StringSplit(names,u_sep,system_names);
   
   Print(system_names[0]);
   for(int k=0; k<ArraySize(system_names); k++)
   {
      //Comment("Running Lleno prediction algo");
      string lowTF = system_names[k]+" lowest_timeframe.txt";
      string lowname = system_names[k]+" lowest_timeframe";
      string midname = system_names[k]+" middle_timeframe";
      string highname = system_names[k]+" highest_timeframe";
      string midTF = system_names[k]+" middle_timeframe.txt";
      string highTF = system_names[k]+" highest_timeframe.txt";
      bool foundlow = true;
      bool foundmid = true;
      bool foundhigh = true;
...
void DrawHistory(string system_name)
{
   // Required variables
   bool foundlow = true;
   bool foundmid = true;
   bool foundhigh = true;
   string lowname = system_name+" lowest_timeframe";
   string midname = system_name+" middle_timeframe";
   string highname = system_name+" highest_timeframe";
   const color lowcolor = clrDarkTurquoise;
   const color midcolor = clrTomato;
   const color highcolor = clrBlanchedAlmond;
   string LowFile = system_name+" Historical data lowest_timeframe.txt";
   string MidFile = system_name+" Historical data middle_timeframe.txt";
   string HighFile = system_name+" Historical data highest_timeframe.txt";
